\begin{figure*}[t]
	...
	\begin{subfigure}[b]{0.32\textwidth}
		\centering
		\begin{tikzpicture}[scale=1.25, >=stealth]
			% Maximal safe set: the constraint set minus the states from which a violation can no longer be
			% avoided. As for a double integrator with a position bound, these form parabolic slivers along
			% the right edge (moving right too fast) and the left edge (moving left too fast); the safe set
			% hugs the constraints and is not ellipsoidal.
			\begin{scope}
				\clip[rounded corners=6pt] (-1.7,-1.1) rectangle (1.7,1.1);
				\fill[red!7] (-1.7,-1.1) rectangle (1.7,1.1);
				\fill[blue!8] (-1.7,1.1) -- (-0.962,1.1)
					-- plot[variable=\t, domain=0:1.1, samples=30] ({1.7-2.2*(1.1-\t)*(1.1-\t)},{1.1-\t})
					-- (1.7,-1.1) -- (0.962,-1.1)
					-- plot[variable=\t, domain=0:1.1, samples=30] ({-1.7+2.2*(1.1-\t)*(1.1-\t)},{-1.1+\t})
					-- cycle;
				\draw[blue!70!black, thin] plot[variable=\t, domain=0:1.1, samples=30] ({1.7-2.2*(1.1-\t)*(1.1-\t)},{1.1-\t});
				\draw[blue!70!black, thin] plot[variable=\t, domain=0:1.1, samples=30] ({-1.7+2.2*(1.1-\t)*(1.1-\t)},{-1.1+\t});
			\end{scope}
			\draw[thick, rounded corners=6pt] (-1.7,-1.1) rectangle (1.7,1.1);
			\node[font=\scriptsize, red!70!black, above right, inner sep=2pt] at (-1.7,-1.1) {$g(x,u)\le 0$};
	...
	\begin{subfigure}[b]{0.32\textwidth}
		...
		\begin{tikzpicture}[scale=1.25, >=stealth]
			\draw[thick, rounded corners=6pt] (-1.7,-1.1) rectangle (1.7,1.1);
			\node[font=\scriptsize, red!70!black, above right, inner sep=2pt] at (-1.7,-1.1) {$g(x,u)\le 0$};
			% the maximal safe set of (d), drawn dashed: the CBF set is an inner approximation of it
	...
	\begin{subfigure}[b]{0.32\textwidth}
		...
		\begin{tikzpicture}[scale=1.25, >=stealth]
			\draw[thick, rounded corners=6pt] (-1.7,-1.1) rectangle (1.7,1.1);
			\node[font=\scriptsize, red!70!black, above right, inner sep=2pt] at (-1.7,-1.1) {$g(x,u)\le 0$};
			% the safe set is never computed: the maximal safe set of (d), drawn dashed
			\draw[gray!60, dashed, thin] plot[variable=\t, domain=0:1.1, samples=30] ({1.7-2.2*(1.1-\t)*(1.1-\t)},{1.1-\t});
			\draw[gray!60, dashed, thin] plot[variable=\t, domain=0:1.1, samples=30] ({-1.7+2.2*(1.1-\t)*(1.1-\t)},{-1.1+\t});
			\node[font=\scriptsize, gray] at (-1.0,-0.38) {implicit};
